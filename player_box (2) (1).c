...
void draw_colourChangeBox(int y);
    ...
    *(pixel_ctrl_ptr + 1) = 0xC0000000;
    ...
    while (1){
		...
		if(count !=0){
			...
			//detect colourChange block
			if(playerPos[1] > colourChangePos -5 && playerPos[1] < colourChangePos + 5){
        ...
        for(i=0;i<4;i++){
			...
			y_circle_pos[i] = y + r * sin(angle_circle[i] * PI / 180.0) + dy*5;
		...
		draw_colourChangeBox(colourChangePos);
