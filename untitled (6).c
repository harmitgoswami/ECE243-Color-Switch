...
void draw_full_triangle_obstacle(int centerX, int centerY, int r, int playerPos[2], int pPlayerPos[2], int xTriangle_pos[3], 
    ...
    int y_triangle = 60; // Y coordinate of circle center
    int r_triangle = 20;
...
int main(void)
{
    ...
    *(pixel_ctrl_ptr + 1) = 0xC0000000;
	...
	for (uint8_t y = 0; y < RESOLUTION_Y; y++) {
    	for (uint8_t x = 0; x < RESOLUTION_X; x++) {
			uint8_t pixel_data = (fb1e3e30fd210af24fb9ecbc6294652b_map[pixel_index] << 8) | fb1e3e30fd210af24fb9ecbc6294652b_map[pixel_index + 1];
			draw_pixel(x,y, pixel_data);
		}
	}
	*/
...
/****************************** PLAYER CONTROL ******************************/		
		...
		//detect colour change box collision
		if(count!=0){
...
/****************************** END OF PLAYER CONTROL ******************************/
		...
		if(drawAllowed){
			if(drawObstacle == 0){
				draw_full_circle_obstacle(circleX, circleY, r, playerPos, pastPlayerPos, x_circle_pos, y_circle_pos, angle_circle, 
							   xp_circle_pos, yp_circle_pos, colours, ballColour, count);
			}else if(drawObstacle == 1){
				draw_full_line_obstacle(playerPos, pastPlayerPos, x_line_pos, y_line_pos, 
							 yp_line_pos, colours, ballColour, overflowColour,count);
			}else if(drawObstacle == 2){
				draw_full_triangle_obstacle(x_triangle, y_triangle,  r, playerPos, pastPlayerPos, x_triangle_pos, 
							   y_triangle_pos, angle_triangle, 
							   xp_triangle_pos, yp_triangle_pos, colours, ballColour, count);
			}
		}
...
void draw_full_triangle_obstacle(int centerX, int centerY, int r, int playerPos[2], int pPlayerPos[2], int xTriangle_pos[3], 
		...
		for(i=0;i<3;i++){
			xTriangle_pos[i] = centerX + r * cos(angleTriangle[i] * PI / 180.0);
			yTriangle_pos[i] = centerY + r * sin(angleTriangle[i] * PI / 180.0);
			angleTriangle[i]+=1;
...
void draw_full_line_obstacle(int playPos[2], int pPlayPos[2], int xLine_pos[4], int yLine_pos, 
	...
	if(count ==0){
		...
		//erase old line
		for(i = ypLine_pos[0] -1; i < ypLine_pos[0] + 5; i++){
		...
		yp_line_pos[0] = yp_line_pos[1];
		yp_line_pos[1] = yp_line_pos[1] + 1;
...
void draw_full_circle_obstacle(int centerX, int centerY, int r, int playPos[2], int pPlayPos[2], int xCircle_pos[4], 
	...
	if(count !=0){
			...
			for(i=0; i<4; i++){
		 		draw_box(xpCircle_pos[i], ypCircle_pos[i], 0x0);
		 	}
        ...
        for(i=0;i<4;i++){
			...
			if(count == 0){
				xCircle_pos[i] = centerX + r * cos(angleCircle[i] * PI / 180.0);
				yCircle_pos[i] = centerY + r * sin(angleCircle[i] * PI / 180.0);
			}else{
				xCircle_pos[i] = centerX + r * cos(angleCircle[i] * PI / 180.0);
				yCircle_pos[i] = centerY + r * sin(angleCircle[i] * PI / 180.0);
			}
		...
		for(i=0;i<4;i++){
			draw_box(xCircle_pos[i], yCircle_pos[i], colours[i]);
		}
