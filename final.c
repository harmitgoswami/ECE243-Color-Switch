...
void write_hex_display(volatile unsigned int *HEX_ptr, int value);
void game_over_screen();
float shortest_distance_to_line_segment(int x, int y, int x1, int y1, int x2, int y2);
...
int main(void)
{
    ...
    *(pixel_ctrl_ptr + 1) = 0xC0000000;
    ...
    while((startGame & 0x01) != 0x1){
        startGame = *sw_ptr;
        //prompt player to press SW0 to start the game
        write_hex_display(HEX3_HEX0_ptr, (U << 24) | (O << 16));
         write_hex_display(HEX5_HEX4_ptr, (S << 8) | U);
    }
...
if(drawAllowed) {
    switch(drawObstacle) {
        ...
        case 1:
            ...
            break;
        case 2:
            draw_full_triangle_obstacle(x_triangle, y_triangle, r_triangle, playerPos, pastPlayerPos, x_triangle_pos, y_triangle_pos, angle_triangle, xp_triangle_pos, yp_triangle_pos, colours, ballColour, count);
...
void draw_full_triangle_obstacle(int centerX, int centerY, int r, int playerPos[2], int pPlayerPos[2], int xTriangle_pos[3], 
		...
		float collision_threshold = 2.0f;
        for (i = 0; i < 3; i++) {
        float distance = shortest_distance_to_line_segment(playerPos[0], playerPos[1], xTriangle_pos[i], yTriangle_pos[i], xTriangle_pos[(i + 1) % 3], yTriangle_pos[(i + 1) % 3]);
        if (distance < collision_threshold && colours[i]!= ballColour) {
            colour = RED;
            game_over_screen();
            break;
        	}
...
void draw_full_line_obstacle(int playPos[2], int pPlayPos[2], int xLine_pos[4], int yLine_pos, 
	...
	if(count ==0){
		...
		//erase old line
		for(i = ypLine_pos[0] -2; i < ypLine_pos[0] + 5; i++){
...
void draw_full_circle_obstacle(int centerX, int centerY, int r, int playPos[2], int pPlayPos[2], int xCircle_pos[4], 
	...
	if(count !=0){
			...
			for(i = 0;i<4;i++){
				if(playPos[0] > xCircle_pos[i] - 12 && playPos[0] < xCircle_pos[i] + 12 && playPos[1] > yCircle_pos[i] -5 
...
void game_over_screen(){
    while(1){
        write_hex_display(HEX3_HEX0_ptr, (I << 24) | (L << 16) | (E << 8) | D);
         write_hex_display(HEX5_HEX4_ptr, (F << 8) | A);
    }
}
...
float shortest_distance_to_line_segment(int x, int y, int x1, int y1, int x2, int y2) {
    ...
    if (l2 == 0.0) return sqrt((x - x1) * (x - x1) + (y - y1) * (y - y1));
